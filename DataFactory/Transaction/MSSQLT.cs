...
namespace NK.Data
{
    ...
    ///  MSSQL事务
    /// </summary>
    internal partial class MSSQLT : IDisposable, iTransaction
    {
        ...
        public MSSQLT(string connection = "", int Timeouts = 60)
        {
            this.Connection = connection;
            this.Timeout = Timeouts;
            ClassName = this.GetType().ToString();
        }
        public MSSQLT(DbConnection Connection)
        { 
            if (Connection != null)
            {
                STConn = Connection as SqlConnection;
        ...
        public bool TableIsExist(string TableName)
        {
            ...
            try
            {
                ...
                cmd.Transaction = ST;
                cmd.Connection = STConn;
                cmd.CommandText = "select count(1) from dbo.sysobjects where id = object_id(N'[dbo].[" + TableName + "]') and OBJECTPROPERTY(id, N'IsUserTable') = 1";
                cmd.CommandTimeout = this.Timeout * 1000;
