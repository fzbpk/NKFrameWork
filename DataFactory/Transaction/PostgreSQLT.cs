...
namespace NK.Data
{
    ...
    /// PostgreSQL事务
    /// </summary>
    internal partial class PostgreSQLT : IDisposable,iTransaction
    {
        ...
        private NpgsqlConnection STConn = null;
        private NpgsqlTransaction ST = null;
        ...
        public PostgreSQLT(DBInfo info)
        {
            DB = info;
            if (DB != null)
            {
        ...
        public PostgreSQLT(string connection = "", int Timeouts = 60)
        {
        ...
        public PostgreSQLT(DbConnection Connection)
        { 
            if (Connection != null)
            {
                STConn = Connection as NpgsqlConnection;
        ...
        public void ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                NpgsqlCommand cmd = STConn.CreateCommand();
                cmd.Transaction = ST;
   ...
   + " where temprownumber>" + ((PageIndex - 1) * PageSize).ToString();
        ...
        public List<string> Columns(string TableName)
        {
            ...
            else
            {
                ...
                { 
                    ...
                    da.SelectCommand.Transaction = ST;
                    da.SelectCommand.CommandTimeout = this.Timeout * 1000;
                    da.SelectCommand.CommandText = "select * from " + TableName + " where 1<>1";
                    da.MissingSchemaAction = MissingSchemaAction.AddWithKey; 
                    da.Fill(ds);
                    dt = ds.Tables[0];
                    for (int i = 0; i < dt.Columns.Count; i++)
        ...
        public void Transaction()
        {
            try
            {
                ...
                STConn = new NpgsqlConnection(this.Connection);
