...
namespace NK.Data
{
    ...
    /// SQLite
    /// </summary>
    public partial class SQLite : DbConnectionHelper, IDisposable,iDataBase
    {
        ...
        private void  init()
        {
            chksql = "select datetime('now')";
            if (Conn == null)
                Conn = new SQLiteConnection(this.Connection);
            initialization();
        }

        public SQLite(DBInfo info):base(info)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
        public SQLite(string connection = "", int Timeouts = 60) : base(connection, Timeouts)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
        ...
        public DbConnection GetConnection()
        {
            ...
            try
            {
                SQLiteConnection conn = new SQLiteConnection(this.Connection);
                conn.DefaultTimeout = this.Timeout * 1000;
                return conn;
            }
            catch (Exception ex)
            {
