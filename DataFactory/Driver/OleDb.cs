...
namespace NK.Data
{
    ...
    /// OLE
    /// </summary>
    public partial class OleDb : DbConnectionHelper, IDisposable, iDataBase
    {
        ...
        public OleDb(DBInfo info):base(info)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
        public OleDb(string connection = "", int Timeouts = 60) : base(connection, Timeouts)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }

        public OleDb(DbConnection Connection) : base(Connection)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
