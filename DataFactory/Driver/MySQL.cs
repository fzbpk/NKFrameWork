...
namespace NK.Data
{
    ...
    /// MySql
    /// </summary>
    public partial class MySql : DbConnectionHelper, IDisposable, iDataBase
    {
        ...
        public MySql(DBInfo info):base(info)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
        public MySql(string connection = "", int Timeouts = 60) : base(connection, Timeouts)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }

        public MySql(DbConnection Connection) : base(Connection)
        {
            dy = DBType.SQLite;
            ClassName = this.GetType().ToString();
        }
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                da.SelectCommand.Connection = (MySqlConnection)Conn;
