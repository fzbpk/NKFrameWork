...
namespace NK.Data
{
    public partial class PostgreSQL : IDisposable, iDataBase
    {
        ...
        public PostgreSQL(string connection = "", int Timeouts = 60)
        {
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                {
                    if (DBConn == null)
                        DBConn = new NpgsqlConnection(this.Connection);
                    conn = DBConn;
                }
                else
                    conn = new NpgsqlConnection(this.Connection);
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new NpgsqlCommand(sql, (NpgsqlConnection)DBConn);
                ...
                {
                    NpgsqlConnection conn = (NpgsqlConnection)GetConnection();
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    conn = (NpgsqlConnection)GetConnection();
                NpgsqlDataAdapter da = new NpgsqlDataAdapter();
                da.SelectCommand = new NpgsqlCommand();
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (NpgsqlConnection)DBConn;
                else
        ...
        { 
            ...
            { 
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new NpgsqlCommand("select count(1) from " + TableName, (NpgsqlConnection)DBConn);
                }
                else
                {
                    conn = (NpgsqlConnection)GetConnection();
                    cmd = new NpgsqlCommand("select count(1) from " + TableName, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
                cmd.ExecuteScalar();
                if (conn != null)
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            { 
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new NpgsqlCommand(CountSql, (NpgsqlConnection)DBConn);
                }
                else
                {
                    conn = (NpgsqlConnection)GetConnection();
                    cmd = new NpgsqlCommand(CountSql , conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
