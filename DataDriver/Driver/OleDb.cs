...
namespace NK.Data
{
    public partial class OleDb : IDisposable, iDataBase
    {
        ...
        public OleDb(string connection = "", int Timeouts = 60)
        {
        ...
        public OleDb(DbConnection Connection)
        { 
            if (Connection != null)
            {
                DBConn = Connection;
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                {
                    if (DBConn == null)
                        DBConn = new OleDbConnection(this.Connection);
                    conn = DBConn;
                }
                else
                    conn = new OleDbConnection(this.Connection);
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OleDbCommand(sql, (OleDbConnection)DBConn);
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    conn = (OleDbConnection)GetConnection();
                OleDbDataAdapter da = new OleDbDataAdapter();
                da.SelectCommand = new OleDbCommand();
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (OleDbConnection)DBConn;
                else
        ...
        public bool TableIsExist(string TableName)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OleDbCommand("select count(1) from " + TableName, (OleDbConnection)DBConn);
                }
                else
                {
                    conn = (OleDbConnection)GetConnection();
                    cmd = new OleDbCommand("select count(1) from " + TableName, conn);
                } 
                cmd.CommandTimeout = this.Timeout * 1000;
                cmd.ExecuteScalar();
                if (conn != null)
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OleDbCommand(CountSql, (OleDbConnection)DBConn);
                }
                else
                {
                    conn = (OleDbConnection)GetConnection();
                    cmd = new OleDbCommand(CountSql, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public bool CheckField(string TableName, string Field, out System.Type FieldType, out bool CanBeNull, out bool IsPrimaryKey)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OleDbCommand("select " + Field + " from " + TableName, (OleDbConnection)DBConn);
                }
                else
                {
                    conn = (OleDbConnection)GetConnection();
                    cmd = new OleDbCommand("select " + Field + " from " + TableName, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
