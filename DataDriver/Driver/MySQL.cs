...
namespace NK.Data
{
    public partial class MySql : IDisposable, iDataBase
    {
        ...
        public MySql(string connection = "", int Timeouts = 60)
        {
        ...
        public MySql(DbConnection Connection)
        { 
            if (Connection != null)
            {
                DBConn = Connection;
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn == null)
                        DBConn = new MySqlConnection(this.Connection);
                    conn = DBConn;
                }
                else
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new MySqlCommand(sql, (MySqlConnection)DBConn);
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (MySqlConnection)DBConn;
                else
        ...
        public bool TableIsExist(string TableName)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new MySqlCommand("select count( * ) FROM information_schema.TABLES WHERE table_name = '" + TableName + "' AND TABLE_SCHEMA = '" + DBConn.Database + "'", (MySqlConnection)DBConn);
                }
                else
                {
                    conn = (MySqlConnection)GetConnection();
                    cmd = new MySqlCommand("select count( * ) FROM information_schema.TABLES WHERE table_name = '" + TableName + "' AND TABLE_SCHEMA = '" + conn.Database + "'", conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new MySqlCommand(CountSql, (MySqlConnection)DBConn);
                }
                else
                {
                    conn = (MySqlConnection)GetConnection();
                    cmd = new MySqlCommand(CountSql, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByDB(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderField, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            Sql = "SELECT " + DisplayField + " FROM  " + TableName + " " + Where + " " + GroupBy + " " + OrderBy + " limit " + ((PageIndex - 1) * PageSize).ToString() + "," + PageSize.ToString();
