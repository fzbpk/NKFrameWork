...
namespace NK.Data
{
    public partial class Oracle : IDisposable, iDataBase
    {
        ...
        public Oracle(string connection = "", int Timeouts = 60)
        {
        ...
        public Oracle(DbConnection Connection)
        { 
            if (Connection != null)
            {
                DBConn = Connection;
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                {
                    if (DBConn == null)
                        DBConn = new OracleConnection(this.Connection);
                    conn = DBConn;
                }
                else
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OracleCommand(sql, (OracleConnection)DBConn);
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (OracleConnection)DBConn;
                else
        ...
        { 
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OracleCommand("SELECT COUNT(*) FROM USER_OBJECTS WHERE UPPER(OBJECT_NAME)='" + TableName.ToUpper() + "'", (OracleConnection)DBConn);
                }
                else
                {
                    conn = (OracleConnection)GetConnection();
                    cmd = new OracleCommand("SELECT COUNT(*) FROM USER_OBJECTS WHERE UPPER(OBJECT_NAME)='" + TableName.ToUpper() + "'", conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            { 
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new OracleCommand(CountSql, (OracleConnection)DBConn);
                }
                else
                {
                    conn = (OracleConnection)GetConnection();
                    cmd = new OracleCommand(CountSql, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByDB(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderField, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            Sql = "SELECT * FROM (SELECT TT.*, ROWNUM AS ROWNO FROM ( SELECT " + DisplayField + " FROM  " + TableName + " " + Where + " " + GroupBy + " " + OrderBy + " ) TT WHERE ROWNUM <= " + PageSize.ToString() + ") TABLE_ALIAS where TABLE_ALIAS.rowno >= " + ((PageIndex - 1) * PageSize).ToString();
