...
namespace NK.Data
{
    public partial class SQLite : IDisposable, iDataBase
    {
        ...
        public SQLite(string connection = "", int Timeouts = 60)
        {
        ...
        { 
            if (Connection != null)
            {
                DBConn = Connection as SQLiteConnection;
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                {
                    if (DBConn == null)
                     DBConn = new SQLiteConnection(this.Connection); 
                    conn = DBConn;
                }
                else
                    conn = new SQLiteConnection(this.Connection);
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SQLiteCommand(sql, (SQLiteConnection)DBConn);
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (SQLiteConnection)DBConn;
                else
        ...
        { 
            ...
            { 
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SQLiteCommand("select count(1) from " + TableName, (SQLiteConnection)DBConn);
                }
                else
                {
                    conn = (SQLiteConnection)GetConnection();
                    cmd = new SQLiteCommand("select count(1) from " + TableName, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
                cmd.ExecuteScalar();
                if (conn != null)
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SQLiteCommand(CountSql, (SQLiteConnection)DBConn);
                }
                else
                {
                    conn = (SQLiteConnection)GetConnection();
                    cmd = new SQLiteCommand(CountSql, conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
