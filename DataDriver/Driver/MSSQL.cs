...
namespace NK.Data
{
    public partial class MSSql : IDisposable, iDataBase
    {
        ...
        public MSSql(DbConnection Connection)
        { 
            if (Connection != null)
            {
                DBConn = Connection;
        ...
        public DbConnection GetConnection()
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn == null)
                        DBConn = new SqlConnection(this.Connection);
                    conn = DBConn;
                }
                else
        ...
        public int ExecuteNonQuery(string sql)
        {
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SqlCommand(sql, (SqlConnection)DBConn);
        ...
        public DataTable getDataTable(string sql, int PageSize, out int RecCount, out int PageCount, string TableName = "")
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                    da.SelectCommand.Connection = (SqlConnection)DBConn;
                else
        ...
        public bool TableIsExist(string TableName)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SqlCommand("select count(1) from dbo.sysobjects where id = object_id(N'[dbo].[" + TableName + "]') and OBJECTPROPERTY(id, N'IsUserTable') = 1", (SqlConnection)DBConn);
                }
                else
                {
                    conn = (SqlConnection)GetConnection();
                    cmd = new SqlCommand("select count(1) from dbo.sysobjects where id = object_id(N'[dbo].[" + TableName + "]') and OBJECTPROPERTY(id, N'IsUserTable') = 1", conn);
                }
                cmd.CommandTimeout = this.Timeout * 1000;
        ...
        public DataTable getDataTableByRam(int PageIndex, int PageSize, string DisplayField, string TableName, string Where, string OrderBy, string GroupBy, out int RecodeCount, out int PageCount)
        {
            ...
            try
            {
                ...
                if (this.KeepAlive)
                {
                    ...
                    if (DBConn.State == ConnectionState.Closed)
                        DBConn.Open();
                    cmd = new SqlCommand(CountSql, (SqlConnection)DBConn);
                }
                else
                {
                    conn = (SqlConnection)GetConnection();
