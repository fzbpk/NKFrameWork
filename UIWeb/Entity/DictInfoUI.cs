...
namespace UIWeb.Entity
{
    ...
    [ToolboxData("<{0}:DictInfoUI runat='server'   ></{0}:DictInfoUI>")]
    public class DictInfoUI : Control, IPostBackDataHandler
    {
        private string Errmsg = "";
        public DictInfo Info { get; set; }
        ...
        public DictInfo Entity
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(json))
                    ...
                    return json.Deserialize<DictInfo>();
            }
            set
            { Info = value; }
        }
        ...
        {
            if (this.Info == null)
                this.Info = new DictInfo();
            Dictionary<PropertyInfo, object> pls = this.Info.ToDictionary();
            string HTML = UIHelper.EntityToForm<DictInfo>(pls, this.UniqueID, LineCSS, HeadCSS, ValueCSS, TextBoxCSS, ComBoxCSS, TimeCSS, CheckBoxCSS);
            writer.Write(HTML);
        }
        ...
        {
            bool res = true;
            Info = UIHelper.FormToEntity<DictInfo>(postDataKey, postCollection, out Errmsg);
