...
namespace UIWeb.Entity
{
    ...
    [ToolboxData("<{0}:PortsSetUI runat='server'   ></{0}:PortsSetUI>")]
    public class PortsSetUI : Control, IPostBackDataHandler
    {
        private string Errmsg = "";
        public PortsSet Info { get; set; }
        ...
        public PortsSet Entity
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(json))
                    ...
                    return json.Deserialize<PortsSet>();
            }
            set
            { Info = value; }
        }
        ...
        {
            if (this.Info == null)
                this.Info = new PortsSet();
            Dictionary<PropertyInfo, object> pls = this.Info.ToDictionary();
            string HTML = UIHelper.EntityToForm<PortsSet>(pls, this.UniqueID, LineCSS, HeadCSS, ValueCSS, TextBoxCSS, ComBoxCSS, TimeCSS, CheckBoxCSS);
            writer.Write(HTML);
        }
        ...
        {
            bool res = true;
            Info = UIHelper.FormToEntity<PortsSet>(postDataKey, postCollection, out Errmsg);
