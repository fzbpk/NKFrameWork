...
namespace UIWeb.Entity
{
    ...
    [ToolboxData("<{0}:ColumnAttributeUI runat='server'   ></{0}:ColumnAttributeUI>")]
    public class ColumnAttributeUI : Control, IPostBackDataHandler
    {
        private string Errmsg = "";
        public ColumnAttribute Info { get; set; }
        ...
        public ColumnAttribute Entity
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(json))
                    ...
                    return json.Deserialize<ColumnAttribute>();
            }
            set
            { Info = value; }
        }
        ...
        {
            if (this.Info == null)
                this.Info = new ColumnAttribute();
            Dictionary<PropertyInfo, object> pls = this.Info.ToDictionary();
            string HTML = UIHelper.EntityToForm<ColumnAttribute>(pls, this.UniqueID, LineCSS, HeadCSS, ValueCSS, TextBoxCSS, ComBoxCSS, TimeCSS, CheckBoxCSS);
            writer.Write(HTML);
        }
        ...
        {
            bool res = true;
            Info = UIHelper.FormToEntity<ColumnAttribute>(postDataKey, postCollection, out Errmsg);
