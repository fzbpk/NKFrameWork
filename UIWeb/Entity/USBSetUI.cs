...
namespace UIWeb.Entity
{
    ...
    [ToolboxData("<{0}:USBSetUI runat='server' ></{0}:USBSetUI>")]
    public class USBSetUI : Control,  IPostBackDataHandler
    {
        private string Errmsg = "";
        public USBSet Info { get; set; }
        ...
        public USBSet Entity
        {
            get
            {
                ...
                if (string.IsNullOrEmpty(json))
                    ...
                    return json.Deserialize<USBSet>();
            }
            set
            { Info = value; }
        }
        ...
        {
            if (this.Info == null)
                this.Info = new USBSet();
            Dictionary<PropertyInfo, object> pls = this.Info.ToDictionary();
            string HTML = UIHelper.EntityToForm<USBSet>(pls, this.UniqueID, LineCSS, HeadCSS, ValueCSS, TextBoxCSS, ComBoxCSS, TimeCSS, CheckBoxCSS);
            writer.Write(HTML);
        }
        ...
        {
            bool res = true;
            Info = UIHelper.FormToEntity<USBSet>(postDataKey, postCollection, out Errmsg);
